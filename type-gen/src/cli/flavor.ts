import {
  ...
  getJsDoc,
  ComponentDeclarationKind, ComponentHeritageClause, ComponentHeritageClauseKind,
  InheritanceResult,
 ...
 */
function extendsEventData(
  node: ts.ClassDeclaration | ts.InterfaceDeclaration,
  checker: ts.TypeChecker
): boolean {
  let current = node;

  if (current.name.getText() === "EventData") return true;
      ...
      for (const clause of heritageClauses) {
        if (clause.token === ts.SyntaxKind.ExtendsKeyword) {
          for (const type of clause.types) {
            ...
            if (symbol && symbol.getName() === "EventData") {
              return true;
            }
            ...
            if (baseClass && extendsEventData(baseClass, checker)) {
              return true;
            }
          }
        }
...
export class NativeScriptFlavor implements AnalyzerFlavor {
  discoverDefinitions(
    ...
    if (
      ...
      const isEventClass = extendsEventData(node as any, context.checker);
      if (isEventClass) {
